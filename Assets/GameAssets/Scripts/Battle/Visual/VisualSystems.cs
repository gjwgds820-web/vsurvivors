    ...
    protected override void OnUpdate()
    {
            ...
            foreach (var entity in entities)
            {
                ...
                if (prefab != null)
                {
                    ...
                    for (int i = 0; i < renderers.Length; i++)
                    {
                        ...
                        {
                            if (renderers[i].sharedMaterial.HasProperty("_MaskTint")) colors[i] = renderers[i].sharedMaterial.GetColor("_MaskTint"); else if (renderers[i].sharedMaterial.HasProperty("_BaseColor"))
        ...
        if (!_portalMissingVisualQuery.IsEmpty)
        {
            ...
            foreach (var entity in entities)
            {
                ...
                var prefab = VisualManager.Instance.GetPortalVisual(CPortalData.PortalID);
                if (prefab == null) continue; // 안전망
                ...
                localTransform.Rotation = prefab.transform.rotation;
                ...
                // 볼텍스 효과가 있다면 스폰 즉시 활성화
                var vortex = go.GetComponentInChildren<Vortex_Controller>();
                if (vortex != null)
                {
                    vortex.F_ToggleVortex(true);
                }
            }
            entities.Dispose();
        }
        ...
        if (!_shadowMissingVisualQuery.IsEmpty)
        {
            ...
            foreach (var entity in entities)
            {
                ...
                {
                    visualId = EntityManager.GetComponentData<ShadowInstanceData>(entity).ShadowID;
                }

                // VisualManager의 Lazy Cache(지연 로딩) 방식을 사용하여 프리팹 가져오기
                var prefab = VisualManager.Instance.GetShadowVisual(visualId);

                if (prefab == null)
                {
                    Debug.LogError($"Shadow visual prefab missing! Visual ID: {visualId}");
                    continue; // 프리팹이 완전히 없으면 스킵하여 에러 방지
                }
                ...
                for (int i = 0; i < renderers.Length; i++)
                {
                    ...
                    if (renderers[i].sharedMaterial != null)
                    {
                        if (renderers[i].sharedMaterial.HasProperty("_MaskTint"))
                            colors[i] = renderers[i].sharedMaterial.GetColor("_MaskTint");
                        else if (renderers[i].sharedMaterial.HasProperty("_MaskTint")) colors[i] = renderers[i].sharedMaterial.GetColor("_MaskTint"); else if (renderers[i].sharedMaterial.HasProperty("_BaseColor"))
                            colors[i] = renderers[i].sharedMaterial.GetColor("_BaseColor");
                        else if (renderers[i].sharedMaterial.HasProperty("_Color"))
                            colors[i] = renderers[i].sharedMaterial.color;
                        else
        ...
        foreach (var (transform, visualModel, entity) in SystemAPI.Query<RefRO<LocalTransform>, SubSceneVisualModel>().WithEntityAccess())
        {
            if (visualModel != null && visualModel.Value != null)
            {
                ...
                else if (distanceSq > 0.0001f || Quaternion.Angle(visualModel.Value.rotation, transform.ValueRO.Rotation) > 0.1f)
                {
                ...
                {
                    // 목표치에 거의 도달했다면 미세한 떨림 방지를 위해 완전히 일치시킴
                    visualModel.Value.position = targetPos;
                    visualModel.Value.rotation = transform.ValueRO.Rotation;
                }
            }
